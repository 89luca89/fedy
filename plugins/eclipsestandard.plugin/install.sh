...
cd "$CACHEDIR"
	...
	PRODUCT=$(echo $PRESENTRELEASE | sed 's/\//-/')


if [[ $(uname -m) == 'x86_64' ]]; then
	URL=$BASEURL/$PRESENTRELEASE/eclipse-java-$PRODUCT-linux-gtk-x86_64.tar.gz
else
	URL=$BASEURL/$PRESENTRELEASE/eclipse-java-$PRODUCT-linux-gtk.tar.gz
fi
